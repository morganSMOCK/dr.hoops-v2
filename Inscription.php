...
<body>
            ...
            <section class="wrapper">
                <div class="form signup">
                  ...
                  <form method="POST" action=""> 
                    ...
                    <input type="text"  placeholder="Adresse email"  id="mail" name="mail"/>
                    ...
                    <a href="login_coach.php"> ESPACE COACH </a></p>
                  </form>
                </div>
                <?php
 if(isset($_POST['envoi'])){
  ...
  if($_POST['mdp'] == $_POST['mdp2']){
    ...
    $sql = 'SELECT * FROM membres WHERE mail = :e';
    $select = $co->prepare($sql);
    $select->execute([
      'e' => $_POST['mail']
    ]);

    $membres = $select->fetch(); // Récupère un résultat

    if(!empty($membres)){
      echo '<p class="result">Cet utilisateur existe déjà</p>';
    }
    else{
      $sql = 'INSERT INTO membres (mail, motdepasse,pseudo) VALUES(:e, :mdp, :p)';
